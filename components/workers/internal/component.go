...
func (c *Component) Run(wg *sync.WaitGroup) (err error) {
	...
	for i := 1; i <= c.config.GetInt(workers.ConfigCount); i++ {
		c.AddWorker()
	}
	...
	go func() {
		...
		c.dispatcher.SetTickerNotifyListenersDuration(c.config.GetDuration(workers.ConfigTickerNotifyListenersDuration))
	}()
...
func (c *Component) GetDispatcher() *dispatcher.Dispatcher {
	return c.dispatcher
}
